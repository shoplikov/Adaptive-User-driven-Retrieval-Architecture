...
class MetricsOverview(BaseModel):
    ...
    positive_satisfaction: float = Field(
        ..., description="Percentage of positive satisfaction scores"
    )
    average_tokens: float = Field(..., description="Average tokens per conversation")
    status_breakdown: Dict[str, int] = Field(
        ..., description="Conversation status breakdown"
    )
...
class SatisfactionMetrics(BaseModel):
    ...
    positive_count: int = Field(
        ..., description="Number of positive satisfaction scores"
    )
    neutral_count: int = Field(..., description="Number of neutral satisfaction scores")
    negative_count: int = Field(
        ..., description="Number of negative satisfaction scores"
    )
    positive_percentage: float = Field(
        ..., description="Percentage of positive satisfaction scores"
    )
    neutral_percentage: float = Field(
        ..., description="Percentage of neutral satisfaction scores"
    )
    negative_percentage: float = Field(
        ..., description="Percentage of negative satisfaction scores"
    )
