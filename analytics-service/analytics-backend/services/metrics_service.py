...
fastapi_backend_dir = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "..", "..", "..", "fastapi_backend")
)
...
spec = importlib.util.spec_from_file_location(
    "analytics_config", os.path.join(analytics_backend_dir, "config", "settings.py")
)
...
spec = importlib.util.spec_from_file_location(
    "fastapi_config", os.path.join(fastapi_backend_dir, "config", "__init__.py")
)
        ...
        try:
            ...
            for row in result:
                ...
                try:
                    ...
                    self.logger.error(
                        f"Error converting row to dict: {conversion_error}"
                    )
                ...
                if (
                    not current_conversation
                    or current_conversation["id"] != conversation_id
                ):
