...
class AnalyticsService:
    ...
    def get_metrics_overview(self, db: Session) -> MetricsOverview:
        ...
        try:
            ...
            positive_count = next(
                (
                    item["count"]
                    for item in satisfaction_stats
                    if item["satisfaction"] == "positive"
                ),
                0,
            )
            positive_percentage = (
                round((positive_count / total * 100), 2) if total > 0 else 0.0
            )
            ...
            avg_tokens = (
                (token_stats["input_tokens"] + token_stats["output_tokens"]) / total
                if total > 0
                else 0.0
            )
    ...
    def get_satisfaction_metrics(self, db: Session) -> SatisfactionMetrics:
        ...
        try:
            ...
            positive_percentage = (
                round((positive_count / total_turns * 100), 2)
                if total_turns > 0
                else 0.0
            )
            neutral_percentage = (
                round((neutral_count / total_turns * 100), 2)
                if total_turns > 0
                else 0.0
            )
            negative_percentage = (
                round((negative_count / total_turns * 100), 2)
                if total_turns > 0
                else 0.0
            )
    ...
    def get_token_usage(self, db: Session) -> TokenUsage:
        ...
        try:
            ...
            if not token_stats:
                return TokenUsage(
                    total_tokens=0, average_tokens=0.0, max_tokens=0, min_tokens=0
                )
    ...
    def get_status_breakdown(self, db: Session) -> StatusBreakdown:
        ...
        try:
            ...
            return StatusBreakdown(
                active=active, inactive=inactive, ended=ended, other=other
            )
